    ...
    let partial_node_data = match expression
    {
        ...
        List(BracketType::Round, elements) => match elements.as_slice()
        {
            ...
            [Symbol(x1), Symbol(name), Symbol(x2), a]
                if x1 == keywords::BINDING && x2 == operators::BINDING_ASSIGN =>
            {
            ...
            // When
            //  (when {...})
            [Symbol(x1), List(BracketType::Curly, inner_elements)] if x1 == keywords::WHEN =>
            {
                context.add_error(
                    errors::INTERNAL,
            ...
            // When-Else
            //  (when {...} else ...)
            [Symbol(x1), List(BracketType::Curly, inner_elements), Symbol(x2), a]
...
fn construct_call_node(
    ...
    mut operands: Vec<ParseItemReference>,
    context: &mut Context,
) -> Option<Node>
{
...
fn construct_type_node<'a>(
    name: String,
    members: Vec<PartialMemberData<'a>>,
    methods: Vec<PartialMethodData<'a>>,
    traits: Vec<String>,
    context: &mut Context,
) -> Option<Node>
{
